...
def verify_password(password: str, stored_hash: str) -> bool:
    try:
        ...
        _, check_salt = hash_password(password, salt)
...
def forgot_password(payload: schemas.ForgotPasswordRequest, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # #region agent log
        _debug_log("auth.py:forgot_password:entry", "Forgot password requested", {"email_domain": email.split("@")[-1] if "@" in email else ""}, "FP1")
        # #endregion
        ...
        if not user or not user.is_active:
            # #region agent log
            _debug_log("auth.py:forgot_password:user", "User not found or inactive (generic response)", {"found": bool(user)}, "FP1")
            # #endregion
            return generic
        ...
        try:
            ...
            # #region agent log
            _debug_log("auth.py:forgot_password:email_error", "Reset email send failed", {"user_id": user.id, "error": str(e)[:180]}, "FP1")
            # #endregion
...
def reset_password(payload: schemas.ResetPasswordRequest, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not prt or prt.used_at is not None or prt.expires_at <= datetime.utcnow():
            # #region agent log
            _debug_log("auth.py:reset_password:invalid", "Reset token invalid/used/expired", {"found": bool(prt)}, "FP2")
            # #endregion
...
def login(login_data: schemas.LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        expires_at = datetime.utcnow() + timedelta(days=7 if login_data.remember_me else 1)
